...
Swift has no built-in database support, but the system's SQLite C library is available for direct import (`import SQLite3`) on both macOS and Linux — no external package needed, since SQLite ships as part of the OS/toolchain. This lesson calls the C API directly, the same layer higher-level wrappers like GRDB.swift are built on.
