...
- Exception for Swift's `11_modules_packages` topic: when compiling multiple Swift files together with `swiftc`, only a file literally named `main.swift` may contain top-level executable statements — every other file must contain only declarations. So the entry point for that one multi-file topic is named `main.swift` instead of `example.swift`/`exercise_1.swift` — a compiler requirement, not a style choice.
